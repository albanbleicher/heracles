<template>
  <div class="intro">
    <div class="regularCursor"></div>
    <transition name="fade"  mode="out-in">
    <div class="introStory" v-show='progression<6'>
      <transition name="fade"  mode="out-in">
        <p class="storyPhrase" v-show='progression>0'>Héraclès, fils du roi des dieux Zeus</p>

      </transition>

      <transition name="fade"  mode="out-in">
      <p class="storyPhrase" v-show='progression>1'>Et de la mortelle Alcmène</p>
      </transition>
      <transition name="fade"  mode="out-in">
        <p class="storyPhrase" v-show='progression>2'>Est connu de tous de nos jours</p>

      </transition>
      <transition name="fade"  mode="out-in">
        <p class="storyPhrase" v-show='progression>3'>Pour l'accomplissement de douze travaux</p>

      </transition>
      <transition name="fade"  mode="out-in">
        <p class="storyPhrase" v-show='progression>4'>Douze tâches impossible à réaliser...</p>
      </transition>
    </div>
  </transition>
  <transition name="fade"  mode="out-in">


    <p v-show='progression>=8'>Mais son histoire ne commence pas là...</p>
  </transition>

    <transition name="fade"  mode="out-in">

      <svg v-show='progression==9' xmlns="http://www.w3.org/2000/svg" width="62" height="88" viewBox="0 0 62 88">
        <g id="Groupe_7" data-name="Groupe 7" transform="translate(-928.885 -726.5)">
          <g id="Groupe_4" data-name="Groupe 4" transform="translate(7)">
            <line id="Ligne_2" data-name="Ligne 2" y2="24" transform="translate(939.5 786.5)" fill="none" stroke="#fff" stroke-width="1"/>
            <line id="Ligne_3" data-name="Ligne 3" y1="9" transform="translate(951.5 786.5)" fill="none" stroke="#fff" stroke-width="1"/>
            <path id="Tracé_2" data-name="Tracé 2" d="M0,50.874v28" transform="translate(963.5 735.626)" fill="none" stroke="#fff" stroke-width="1"/>
          </g>
          <g id="Groupe_1" data-name="Groupe 1" transform="translate(393.885 -74.115)">
            <text id="SCROLL" transform="translate(566 854)" fill="#fff" font-size="12" font-family="ArialMT, Arial" letter-spacing="0.2em"><tspan x="-30.009" y="0">SCROLL</tspan></text>
          </g>
          <g id="Groupe_4-2" data-name="Groupe 4" transform="translate(7)">
            <line id="Ligne_2-2" data-name="Ligne 2" y2="21" transform="translate(939.5 744.5)" fill="none" stroke="#fff" stroke-width="1"/>
            <line id="Ligne_3-2" data-name="Ligne 3" y2="39" transform="translate(951.5 726.5)" fill="none" stroke="#fff" stroke-width="1"/>
            <line id="Ligne_4" data-name="Ligne 4" y2="29" transform="translate(963.5 736.5)" fill="none" stroke="#fff" stroke-width="1"/>
          </g>
        </g>
      </svg>


</transition>


  </div>
</template>
<script type="text/javascript">
export default {
  data() {
    return {
      progression:0
    }
  },
  methods:{
    startStory() {
      var self=this;
        var story = setInterval(function(){
        self.progression++;
        console.log(self.progression);
        if(self.progression == 7){
          clearInterval(story);
          setTimeout(function(){
            var final = setInterval(function(){
              if(self.progression<9) {
                self.progression++
              }
              else {
                clearInterval(final)
              }
            },1500)
          },3000)
          document.addEventListener("wheel", startFullStory);
        }
        function startFullStory() {
          var chapter =1;
          document.removeEventListener("wheel", startFullStory);

          self.$router.push({ name: 'story', params: { chapter } })

        }
      },2500)
    }
  },
  mounted() {
    var self = this;
    document.title='Héraclès - Introduction';
    document.addEventListener('mousemove', function(e) {
      console.log(e);
      document.querySelector('.regularCursor').style.left = (e.clientX - 4)+'px' ;
      document.querySelector('.regularCursor').style.top = (e.clientY - 4)+'px';
    })
    self.startStory();
  }
}
</script>
